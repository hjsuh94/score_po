...
from score_po.policy_optimizer import (
    DRiskScorePolicyOptimizer, DRiskScorePolicyOptimizerParams)


class GymPolicyOptimizer(DRiskScorePolicyOptimizer):
    def __init__(self, params: DRiskScorePolicyOptimizerParams,
                 env_name: str, **kwargs):
...
class GymPolicyEvaluator:
    ...
    def get_policy_score(self):
        ...
        while not done:
            action = self.policy(
                torch.Tensor(obs)[None,:], time)[0].detach().numpy()
