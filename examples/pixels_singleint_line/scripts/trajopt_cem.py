...
from score_po.dynamical_system import NNDynamicalSystem_Image, NNEnsembleDynamicalSystem
...
@hydra.main(config_path="../config", config_name="trajopt_cem")
def main(cfg: DictConfig):
    ...
    params.cost = cost
    # y_mesh, x_mesh = torch.meshgrid(torch.linspace(-1, 1, 32), torch.linspace(-1, 1, 32))
    # y_mesh_u, x_mesh_u = torch.meshgrid(torch.linspace(-0.2, 0.2, 32), torch.linspace(-0.2, 0.2, 32))
    ...
    def plot_trj_small(x_trj, u_trj, iter, file_name='', colormap_name='winter'):
        if not isinstance(x_trj, np.ndarray):
            x_trj_orig, u_trj_orig = x_trj.detach().cpu().numpy(), u_trj.detach().cpu().numpy()
    ...
    # 5. Run
    optimizer = CEMEnsembleImage(params)
    ...
    plot_trj_small(x_trj_hal[0::5], u_trj_hal[[0, 5, 10, 14]], 0, file_name="hallucinated_")
