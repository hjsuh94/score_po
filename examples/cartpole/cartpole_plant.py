...
class CartpoleNNDynamicalSystem(NNDynamicalSystem):
    ...
    def __init__(
        ...
        """
        layers = []
        layer_in_features = 5  # The input is (x, u)
        for i in range(len(hidden_widths)):
            layers.append(
                torch.nn.Linear(layer_in_features, hidden_widths[i], device=device)
            )
            layers.append(torch.nn.LeakyReLU())
            layer_in_features = hidden_widths[i]
        layers.append(torch.nn.Linear(hidden_widths[-1], 4, device=device))

        residual_net = torch.nn.Sequential(*layers)
        self.device = device
